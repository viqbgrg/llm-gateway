...
public class RuntimeHealthService {
    ...
    public Mono<HealthView> all() {
        return Mono.zip(bindings.findAll().concatMap(this::binding).collectList(),
    ...
    public Mono<BindingView> binding(String id) {
        return bindings.findById(id).switchIfEmpty(Mono.error(new org.springframework.web.server.ResponseStatusException(
                org.springframework.http.HttpStatus.NOT_FOUND, "Binding not found"))).flatMap(this::binding);
    }

    public Mono<DashboardView> dashboard() {
        return Mono.zip(dashboard.read(), bindings.findAll().concatMap(this::binding).collectList())
    ...
    private Mono<BindingView> binding(BindingEntity binding) {
        return configurations.inspect(binding.virtualModelId()).flatMap(configuration -> {
            var candidate = configuration.candidates().stream().filter(c -> c.binding().id().equals(binding.id())).findFirst().orElseThrow();
