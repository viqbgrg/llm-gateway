...
同日后续缺陷修复与生产基础完善：后端增至 24 个套件共 459 项，另有官方 Anthropic SDK 的 8 个流式用例通过；修复编码路径鉴权绕过、流式 usage 缺失和 Responses 历史回传问题。管理/推理密钥分离、Redis 鉴权、存储端口隔离、配置批量读取和前端按需加载已完成，4 个浏览器场景及 78 次有界负载再次通过。CI 和升级说明已补齐，数据库仍为 V5。
